...
def ransac(T):
    ...
    for p in maxInliers:
        p1 = [p[0], p[1], 1]
        p2 = [p[2], p[3], 1]
        A.append([p1[0], p1[1], 1, 0, 0, 0, -1 * p2[0] * p1[0], -1 * p2[0] * p1[1], -1 * p2[0]])
        A.append([0, 0, 0, p1[0], p1[1], 1, -1 * p2[1] * p1[0], -1 * p2[1] * p1[1], -1 * p2[1]])
    A = np.matrix(A)
    u, s, v = np.linalg.svd(A)
    # print("====",v.shape)
    h = np.reshape(v[8], (3, 3))
...
transfered = cv2.warpAffine(img_1, M, (512,512), borderValue=(255, 255, 255))
